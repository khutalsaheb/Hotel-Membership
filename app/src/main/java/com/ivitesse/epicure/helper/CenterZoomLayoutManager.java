...
*/
    ...
    public CenterZoomLayoutManager(@NonNull Context context, int orientation, boolean reverseLayout) {
    ...
    public int scrollVerticallyBy(int dy, @NonNull RecyclerView.Recycler recycler, @NonNull RecyclerView.State state) {
        ...
        if (orientation == VERTICAL) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                float childMidpoint =
                        (getDecoratedBottom(Objects.requireNonNull(child)) + getDecoratedTop(child)) / 2.f;
    ...
    public int scrollHorizontallyBy(int dx, @NonNull RecyclerView.Recycler recycler, @NonNull RecyclerView.State state) {
        ...
        if (orientation == HORIZONTAL) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                View child = getChildAt(i);
                float childMidpoint =
                        (getDecoratedRight(Objects.requireNonNull(child)) + getDecoratedLeft(child)) / 2.f;
                float d = Math.min(d1, Math.abs(midpoint - childMidpoint));
                float scale = s0 + (s1 - s0) * (d - d0) / (d1 - d0);
                child.setScaleX(scale);
                child.setScaleY(scale);
